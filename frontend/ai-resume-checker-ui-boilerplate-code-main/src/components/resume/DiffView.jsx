...
function GradientNumber({ value, size = 32, palette = "sage" }) {
  const gradient =
    palette === "danger"
      ? "linear-gradient(135deg, #FCA5A5 0%, var(--danger) 50%, #B91C1C 100%)"
...
export function DiffView({ resumeId, versions }) {
  ...
  return (
    <Card>
      ...
      {error && (
        <div className="text-xs text-[var(--danger)] bg-[#FEE2E2] rounded-xl px-3 py-2">
          {error.message}
        </div>
      )}
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
            <div className="h-10 px-4 flex items-center justify-between border-b border-[var(--border)] bg-[var(--surface-2)]/60">
              ...
              <div className="flex items-center gap-3 text-[10px] text-[var(--ink-muted)]">
                ...
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-sm bg-[#FEE2E2] border border-[var(--danger)]/30" />
                  Removed
                </span>
              </div>
            </div>
              ...
              {data.parts.map((p, i) => (
                <span
                  ...
                  className={cn(
                    ...
                    p.removed &&
                      "bg-[#FEE2E2] text-[var(--danger)] line-through decoration-[var(--danger)]/50 rounded px-0.5 py-0.5"
                  )}
                >
                  {p.value}
                </span>
