  ...
  return (
    <div className="hidden lg:block flex-1 relative rounded-[28px] overflow-hidden isolate">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(140deg, #0F172A 0%, #1E40AF 38%, #1E293B 72%, #0F172A 100%)",
        }}
      />
...
export function AuthPrimaryButton({ children, disabled, ...props }) {
  return (
    <motion.button
      ...
      disabled={disabled}
      className="relative w-full h-12 rounded-2xl text-white font-semibold text-[15px] flex items-center justify-center gap-2 overflow-hidden shadow-[0_8px_24px_-8px_rgba(30,64,175,0.55)] transition-all duration-200 hover:shadow-[0_12px_28px_-8px_rgba(30,64,175,0.7)] disabled:opacity-60 disabled:cursor-not-allowed"
      style={{
        background:
...
export function AuthErrorBanner({ children }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="text-xs text-[var(--danger)] bg-[#FEE2E2] rounded-2xl px-4 py-2.5 leading-snug"
    >
      {children}
    </motion.div>
  );
